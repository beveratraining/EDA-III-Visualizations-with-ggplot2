...
```{r}
ggplot(yeast) + 
  geom_point(mapping = aes(x=alm, y= gvh))
```
...
#this is the same as line 35-36 without the "labels"
#ggplot(yeast) + 
#  geom_point(aes(alm,gvh, color=ClassDistribution))
```

```{r}
ggplot(yeast) + 
  geom_histogram(mapping = aes(x=gvh, color=ClassDistribution), bins=5, fill='white',  position='dodge')
```
