...
ggplot(yeast) + 
  geom_point(mapping = aes(x=alm, y= gvh, color=ClassDistribution))
...
y4 <- filter(y2, bacteria %in% c("alm","mit","erl"))
...
ggplot(groups, aes(bacteria, measurements)) +   stat_boxplot(geom = "errorbar",
               # Error bars
               width = 0.25) +    
              # Bars width
